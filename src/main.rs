...
fn main() {
    App::new()
        ...
        .add_systems(
            (
                clear.run_if(|config: Res<Config>| config.auto_update),
...
fn clear(mut commands: Commands, generated_q: Query<Entity, With<Generated>>) {
    for gen in generated_q.iter() {
        commands.entity(gen).despawn();
    }
}
